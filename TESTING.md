...
The unit tests mock the in-repo Substack client setup to test node logic without any HTTP dependencies.

### Mock Components

1. **Mock Substack Client** (`tests/mocks/mockSubstackClient.ts`)
   - Mocks the in-repo Substack client returned by `SubstackUtils.initializeClient()`
...
```typescript
import { Substack } from '../../nodes/SubstackGateway/SubstackGateway.node';
