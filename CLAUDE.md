...
- **Credentials**: `credentials/SubstackGatewayApi.credentials.ts` - API authentication configuration
...
- **axios / axios-rate-limit**: HTTP client and request throttling for the in-repo Substack client
...
- **Mock Strategy**: Mock `SubstackUtils.initializeClient()` and the in-repo client methods
- **Mock Components**:
